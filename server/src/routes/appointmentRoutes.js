...
async function denormalize(body) {
  ...
  if (body.doctor) {
    ...
    if (doctor) {
      next.doctorName = doctor.name;
      if (!body.department) next.department = doctor.department || doctor.specialty;
    }
  }
  return next;
}

const ctrl = crudController(Appointment, {
  searchFields: ['patientName', 'doctorName', 'department', 'reason', 'status'],
